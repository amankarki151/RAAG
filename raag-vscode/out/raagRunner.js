...
exports.RaagCliError = void 0;
exports.analyzeWorkspace = analyzeWorkspace;
exports.getConfiguredCliPath = getConfiguredCliPath;
const vscode = require("vscode");
const cp = require("child_process");
const fs = require("fs");
const os = require("os");
const path = require("path");
class RaagCliError extends Error {
    stderr;
...
exports.RaagCliError = RaagCliError;
 ...
 */
async function analyzeWorkspace(workspaceFolder, cliPath) {
    ...
    try {
        const sampleResult = await run(cliPath, ["sample", "run", ".", "--output", snapshotPath, "--quiet"], workspaceFolder);
        if (sampleResult.code === 2) {
            ...
            // re-worded here.
            throw new RaagCliError("raag sample run failed. Is the Sample Engine built?", sampleResult.stderr);
        }
        const tuneResult = await run(cliPath, ["tune", "run", snapshotPath, "--export-metrics", metricsPath], workspaceFolder);
        ...
        if (!fs.existsSync(metricsPath)) {
            throw new RaagCliError("raag ran but did not produce a metrics report.", tuneResult.stderr);
        }
        ...
        if (nodeError.code === "ENOENT") {
            throw new RaagCliError(`Could not find '${cliPath}'. Set raag.cliPath in settings, or activate the virtual environment RAAG is installed in before launching VS Code.`, "");
        }
        throw error;
    }
    finally {
...
function getConfiguredCliPath() {
    return vscode.workspace.getConfiguration("raag").get("cliPath", "raag");
}
//# sourceMappingURL=raagRunner.js.map